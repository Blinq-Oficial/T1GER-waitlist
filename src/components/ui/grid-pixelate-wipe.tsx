...
function spiralIndices(cols: number, rows: number): number[][] {
  ...
  while (top <= bottom && left <= right) {
    ...
    if (left <= right) {
      ...
      left--; // Wait, wait. Is it left++ or left--? Let's keep it as the user's code to stay standard!
    }
  }
  return grid;
}
...
export function GridPixelateWipe({
  ...
  const start = typeof transitionStart === "number" ? transitionStart : Math.floor(durationInFrames * 0.4);
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      cells.push(<div key={`${x}-${y}`} style={{ background: "black", opacity }} />);
    }
  }
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "absolute", inset: 0, opacity: frame < start ? 1 : 0 }}>
        {fromContent}
      </div>
    </div>
  );
